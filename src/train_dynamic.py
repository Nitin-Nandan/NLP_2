...
def evaluate(model, dataloader, device):
    ...
    # Crucially, deactivate dynamic dropout during evaluation!
    # It must only be applied dynamically during training!
    ...
    # Reactivate dropout for any subsequent training epochs
...
if __name__ == "__main__":
    ...
    print("Initiating Phase 4 Dynamic Tokenizer Training Execution via ML-Scientist")
    
    tokenizer_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'dynamic_tokenizer.json')
    try:
        ...
        # Explicit Dynamic Configuration (BPE-Dropout directly in backend Model)
        print("Activating Dynamic Dropout BPE Mapping (p=0.1)...")
    ...
    manager = CoreModelManager(num_labels=2, new_vocab_size=10000)
    ...
    print("Evaluating Dynamic-Tokenizer Model...")
    val_acc, val_f1 = evaluate(model, val_loader, device)
    total_time = time.time() - start_time
    print(f"Dynamic-Tokenizer Training complete! Acc: {val_acc:.4f} | F1: {val_f1:.4f} | Time: {total_time:.2f}s")
    print(f"DYNAMIC_FINAL_METRICS|{val_acc:.4f}|{val_f1:.4f}|{total_time:.2f}|{params_count}")
