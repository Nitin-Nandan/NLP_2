...
if __name__ == "__main__":
    ...
    print("Initiating Phase 3 Char-Tokenizer Training Execution via ML-Scientist")
    
    tokenizer_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'char_tokenizer.json')
    try:
    ...
    train_loader, val_loader = factory.get_dataloaders(tokenizer, batch_size=32, max_length=256)
    
    manager = CoreModelManager(num_labels=2, new_vocab_size=71)
    ...
    print("Evaluating Char-Tokenizer Model...")
    val_acc, val_f1 = evaluate(model, val_loader, device)
    total_time = time.time() - start_time
    print(f"Char-Tokenizer Training complete! Acc: {val_acc:.4f} | F1: {val_f1:.4f} | Time: {total_time:.2f}s")
    print(f"CHAR_FINAL_METRICS|{val_acc:.4f}|{val_f1:.4f}|{total_time:.2f}|{params_count}")
