...
if __name__ == "__main__":
    ...
    print("Initiating Phase 4 Hybrid Tokenizer Training Execution via ML-Scientist")
    
    tokenizer_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'hybrid_tokenizer.json')
    try:
    ...
    manager = CoreModelManager(num_labels=2, new_vocab_size=5000)
    ...
    print("Evaluating Hybrid-Tokenizer Model...")
    val_acc, val_f1 = evaluate(model, val_loader, device)
    total_time = time.time() - start_time
    print(f"Hybrid-Tokenizer Training complete! Acc: {val_acc:.4f} | F1: {val_f1:.4f} | Time: {total_time:.2f}s")
    print(f"HYBRID_FINAL_METRICS|{val_acc:.4f}|{val_f1:.4f}|{total_time:.2f}|{params_count}")
